    ...
    <draggable
        ...
        <template #item="{element}">
            <div :class="['card', 'with-header', element.active ? 'open' : '']">
                <div class="header">
                    <div class="title">{{ element.name ? element.name : trans.newVehicleModel }}</div>
                    <div class="controls">
                        <div class="button" @click="closeVehicleModel(element)" v-if="element.active">
                            <i class='bx bx-chevron-up'></i>
                        </div>
                        <div class="button" @click="openVehicleModel(element)" v-if="!element.active">
                            <i class='bx bx-chevron-down'></i>
                        </div>
                        <div class="button"
                             @click="openDeleteConfirmPopup('removeVehicleModel', 'vehicle-model', element.id, element.name ? element.name : trans.newVehicleModel)">
                            <i class='bx bx-x'></i>
                        </div>
                    </div>
                </div>
                <div class="content" v-if="element.active">
                    ...
                    <vehicle-model-name :vehicleModelNames="element.names" :removeLanguagePageBlockOrItem="removeLanguagePageBlockOrItem"></vehicle-model-name>

                    <div class="row">
                        <div class="full">
                            <button class="primary mb-1" @click="addVehicleModelName(element.id)">{{ trans.addVehicleModelName }}</button>
                        </div>
                    </div>
                </div>
            </div>
...
import VehicleModelName from './VehicleModelName.vue';
...
export default {
    ...
    data() {
        return {
            ...
            trans: {
                name: 'Name',
                newVehicleModel: 'New Model',
                addVehicleModelName: 'Add Model Name',
            }
        }
    },
    components: {
    ...
    // },
    methods: {
        openVehicleModel(vehicleModel) {
            vehicleModel.active = true;
        },

        addVehicleModelName(vehicleModelId) {
            let findVehicleModelIndex = this.local_data.findIndex(item => item.id === vehicleModelId);

            if (findVehicleModelIndex >= 0) {
                this.local_data[findVehicleModelIndex].names.push({
        ...
        closeVehicleModel(vehicleModel) {
            vehicleModel.active = false;
        },
    ...
    mounted() {
        this.local_data = this.vehicleModels;
    }
}
</script>
