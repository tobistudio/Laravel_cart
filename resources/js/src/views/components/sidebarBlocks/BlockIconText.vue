            ...
            <template #item="{element}">
                <div :class="['card', 'with-header', element.active ? 'open' : '']">
                    ...
                    <div class="content" v-if="element.active">
                        <div class="row">
                            <div class="full">
                                <div :class="['input-wrapper', 'image', icon_image_validation ? 'error' : '', iconImageClassFocus]">
                                    <input @change="iconImageUpload($event, element)" type="file" accept="image/*" class="image" :id="['iconImage-' + element.id]" ref="iconImage">
                                    <div class="image-placeholder">
                                        <div class="label">{{ trans.icon_image }}</div>
                                        <div class="label-more">{{ trans.icon_image_description }}</div>
                                        <div class="image" @click="triggerIconImageUpload('iconImage-' + element.id)">
                                            <div class="placeholder" v-if="!element.icon_image"></div>
                                            <img :src="element.icon_image" :alt="trans.icon_image" v-if="element.icon_image">
                                        </div>
                                    </div>
                                    <span class="error" v-if="icon_image_validation">{{ icon_image_validation }}</span>
                                </div>
                            </div>
                        </div>
                        <div class="row">
...
export default {
    ...
    data() {
        return {
            ...
            iconImageClassFocus: false,
            icon_image_validation: null,
            titleClassFocus: false,

            trans: {
                icon_image: 'Icon image',
                icon_image_description: 'Minimum image size 30x30px, please use 1:1 aspect ratio otherwise image will be cropped',
                title: 'Title',
                add_item: 'Add Item',
                new_item: 'New Item',
            }
    ...
    watch: {
        local_data: {
            handler(oldData, newData) {
                if (newData) {
                    this.$emit('sidebarBlockUpdated', this.block.id, this.block.tmp_id, this.local_data, null);
                }
            },
            deep: true,
            immediate: true,
    ...
    methods: {
        ...
        triggerIconImageUpload(id) {
            document.getElementById(id).click();
        },
        iconImageUpload(event, element) {
            let reader = new FileReader()
            reader.readAsDataURL(event.target.files[0])
            reader.onload = () => {
                element.icon_image = reader.result;
            };
        },
        titleFocus() {
