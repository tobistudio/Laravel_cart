<!-- =========================================================================================
  File Name: BlockNavigation.vue
  Description: Block Navigation
========================================================================================== -->
<template>
            ...
            <template #item="{element}">
                <div :class="['card', 'with-header', element.active ? 'open' : '']">
                    ...
                    <div class="content" v-if="element.active">
                        ...
                        <div class="row">
                            <div class="full">
                                <div class="parent-pages">
                                    <WDSelect field="selectedPage" label="name" :item_id="element.id" :title="trans.page" :options="pagesOptions" :selectedOption="element.page"
                        ...
                        <div class="row">
                            <div class="full">
                                <div :class="['input-wrapper', element.link ? 'has-content' : '', linkClassFocus]">
                                    <div class="label">{{ trans.link }}</div>
                                    <div class="label-floating">{{ trans.link }}</div>
                                    <input v-model="element.link" type="text" @focus="linkFocus" @blur="linkBlur"/>
                                </div>
                            </div>
                        </div>
                    </div>
...
export default {
    ...
    methods: {
        ...
        selectedOptionUpdated(option, field, item_id) {
            let findMenuItemIndex = this.local_data.items.findIndex(item => item.id === item_id);

            if(findMenuItemIndex >= 0){
                this.local_data.items[findMenuItemIndex].page = option;
            }
        },
        closeItem(block) {
            block.active = false;
        ...
        linkFocus() {
            this.linkClassFocus = 'focus';
        },
        linkBlur() {
            this.linkClassFocus = false;
        },
    },
    mounted() {
